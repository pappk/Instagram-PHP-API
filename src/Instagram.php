...
class Instagram {
	...
	public function __construct( $config ) {
		if ( is_array( $config ) ) {
			...
			$this->setProxyHost( $config['proxyHost'] );
			$this->setProxyPort( $config['proxyPort'] );
			$this->setProxyUser( $config['proxyUser'] );
			$this->setProxyPass( $config['proxyPass'] );
	...
	protected function _makeCall( $function, $params = null, $method = 'GET' ) {
		...
		if ( ! empty( $this->getProxyHost() ) && ! empty( $this->getProxyPort() ) ) {
			...
			if ( ! empty( $this->getProxyUser() ) && ! empty( $this->getProxyPass() ) ) {
